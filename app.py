...
def data():
    ...
    for row in ws.iter_rows(min_row=total_rows-3, max_row=total_rows, values_only=True):
        data.append(row)
        ...
        try:
            ...
            if line != '':
                ...
                if(line[0]=='T'):
                    float_values = re.findall(r"\d+\.\d+", line)
                    float_values = [float(value) for value in float_values]
                    temperature=20.0 #float_values[0]
                    print(temperature)

                # elif(line[0]=='I'):
                    # float_values = re.findall(r"\d+\.\d+", line)
                    # float_values = [float(value) for value in float_values]
                    irradiance=float_values[1]
                    print(irradiance)
                elif(line[0]=='V'):
                    float_values = re.findall(r"\d+\.\d+", line)
                    float_values = [float(value) for value in float_values]
                    voltage=float_values[0]
                    print(voltage)
                elif(line[0]=='C'):
                    float_values = re.findall(r"\d+\.\d+", line)
                    float_values = [float(value) for value in float_values]
                    current=10.0 #float_values[0]
                    print(current)
                    print(temperature,irradiance,voltage,current)
                    oc_voltage=1*(20.1+(-0.0035)*(temperature-30))

                    oc_est=1*(21.77+(-0.0035)*(temperature-25))
                    voltage_est=1*(18.14+(-0.0035)*(temperature-25))
                    current_est=2*((7.17*irradiance/1000)+(-0.0005)*(temperature-25))
                    ir, vr, ocr = ratio.calculate_ratios(current, current_est,voltage, voltage_est, oc_voltage, oc_est)
                    ...
                    ws.append([voltage,current,voltage*current,temperature,irradiance,current_time,rules[max_rule]])
                    wb.save("data.xlsx")
